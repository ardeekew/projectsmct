...
const AddUserModal = ({ modalIsOpen, closeModal, openCompleteModal, entityType }: { modalIsOpen: boolean, closeModal: any, openCompleteModal: any, entityType:string }) => {
      if (!modalIsOpen) {
    return null;
  }
...
return (
    ...
    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50 flex-col ">
           ...
           <h2 className='text-center  text-xl md:text-[32px] font-bold text-white'>Add {entityType}</h2>
      <XMarkIcon className='size-6 text-black absolute right-3 cursor-pointer' onClick={closeModal}/>
    </div>
    ...
    <div className='grid lg:grid-cols-2 justify-items-center  place-content-center mt-10 mx-10 gap-2'>
      ...
      <div className='flex justify-center lg:justify-end items-center space-x-2 md:mt-20 md:mr-10 mb-10'>
          <button className='bg-[#9C9C9C] p-2 lg:w-1/4 rounded-[12px] text-white font-medium'onClick={closeModal}>Cancel</button>
          <button className='bg-primary p-2 lg:w-1/4  rounded-[12px] text-white font-medium' onClick={openCompleteModal}>Add</button>
          </div>
      </div>
  </div>
    )
