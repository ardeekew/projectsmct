...
const CompleteModal = ({ showCompleteModal, closeCompleteModal, openCompleteModal, entityType }: { showCompleteModal: boolean, closeCompleteModal: any, openCompleteModal: any, entityType: string }) => {
...
return (
    ...
    <p className='my-7  text-gray-400 font-semibold text-center'>{entityType} Added!</p>
   
    </div>
    <div className='bg-graybg w-full rounded-b-lg flex justify-center items-center p-4'>
    <button className=' bg-primary p-2 w-1/2 rounded-[12px] text-white font-extrabold' onClick={closeCompleteModal}>OKAY</button>
      </div>
    </div>
    </div>
)
